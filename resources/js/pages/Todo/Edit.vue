<template>
  ...
  <div class="sm:mx-auto sm:w-full sm:max-w-sm">
    <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">Edit Todo</h2>
  </div>
...
const submit = async () => {
  try {
    ...
    const res = await form.put(route("todos.update", {todo: todo.value.id}));

    emitter.emit(`show-success-notification`, "Todo successfully updated");
...
const getTodo = async()=> {
    try {
        const {data} = await axios.get(route('todos.show', {todo: vRoute.params.id}))

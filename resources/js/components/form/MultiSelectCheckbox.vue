...
const selectedItems = computed(() => {
  return props.options.filter((item) => {
    return props.modelValue.includes(item[props.valueKey]);
  });
});

const selected_item_label = computed(() => {
  if (selectedItems.value.length) {
    let text = props.stop_translation
      ? selectedItems.value[0][props.labelKey]
      : trans(selectedItems.value[0][props.labelKey].toString());
    if (selectedItems.value.length > 1) {
      text += ` +${selectedItems.value.length - 1}`;
    }
...
const isSelected = (value) => {
  return props.modelValue.find((item) => item == value);
};
...
.selected_item_label {
  @apply text-[13px] overflow-hidden flex gap-2;

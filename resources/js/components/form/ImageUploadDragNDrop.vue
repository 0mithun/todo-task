<template>
  <div class="input-element">
    ...
    <div class="attachements w-full">
      <label
        :class="$attrs.attachmentlabelClass"
        class="file-input flex flex-col px-4 py-12 z-10 cursor-pointer bg-white"
    ...
    <div
      class="flex flex-wrap flex-grow gap-6 overflow-x-scroll rounded-md files mt-3 pt-3"
    >
      <template v-if="props.showFileNames">
        <div
          class="flex flex-wrap gap-1 group"
        ...
        >
          <div class="relative">
            ...
            <RemoveIcon
              ...
              @mouseover="focused = true"
              class="absolute cursor-pointer text-gray-500 hover:text-gray-700 right-[-15px] top-[-12px] w-4 h-4 p-1 bg-white rounded-lg"
            />
          </div>
        </div>
      </template>
...
const addFiles = () => {
  ...
  if (
    ...
    if (fileSizeExceeds) {
      localError.value = trans("The size limit is :size MB for each file.", {
